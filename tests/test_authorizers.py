...
def test_creating_api_token_authorizer(my_data: MyData) -> None:
    ...
    api_token_authorizer = APITokenAuthorizer(
        my_data_object=my_data, authorizer=invalid_token_authroizer
    )
    assert (
        invalid_token_authroizer._api_token_authorizer is api_token_authorizer
    )
...
def test_setting_api_token_authorizer_again(my_data: MyData) -> None:
    ...
    with pytest.raises(APITokenAuthorizerAlreadySetError):
        invalid_token_authroizer.set_api_token_authorizer(api_token_authorizer)


@pytest.mark.parametrize(
    'api_token', ['invalid_token_1', 'invalid_token_2', None]
)
def test_invalid_token_authorizer_invalid_token(
    my_data: MyData, api_token: str | None
) -> None:
...
@pytest.mark.parametrize('api_token', ['aRlIytpyz61JX2TvczLxJZUsRzk578pE'])
...
@pytest.mark.parametrize('api_token', ['2e3n4RSr4I6TnRSwXRpjDYhs9XIYNwhv'])
...
def test_api_token_authorizer_get_api_token_empty_user(
    ...
    """
    authorizer = APITokenAuthorizer(
        my_data_object=my_data, authorizer=ShortLivedTokenAuthorizer()
    )
...
@pytest.mark.parametrize('api_token', ['aRlIytpyz61JX2TvczLxJZUsRzk578pE'])
def test_api_token_authorizer_is_root(my_data: MyData, api_token: str) -> None:
...
@pytest.mark.parametrize(
    'api_token', ['2e3n4RSr4I6TnRSwXRpjDYhs9XIYNwhv', None]
)
...
@pytest.mark.parametrize(
    'api_token', ['aRlIytpyz61JX2TvczLxJZUsRzk578pE', None]
)
...
@pytest.mark.parametrize(
    'api_token', ['MHxHL4HrmmJHbAR1b0gV4OkpuEsxxmRL', None]
)
def test_disabled_token(my_data: MyData, api_token: str) -> None:
    ...
    """
    authorizer = APITokenAuthorizer(
        my_data_object=my_data, api_token=api_token
    )
    assert authorizer.is_enabled is False


@pytest.mark.parametrize(
    'api_token', ['Cbxfv44aNlWRMu4bVqawWu9vofhFWmED', None]
)
def test_expired_token(my_data: MyData, api_token: str) -> None:
